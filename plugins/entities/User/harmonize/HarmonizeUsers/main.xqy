...
import module namespace tfs-flow = "http://example.com/ext/data-hub-framework/impl/flow-lib" at "/ext/data-hub-framework/impl/flow-lib.xqy";
  ...
  $options as map:map)
{
  ...
  let $envelope := tfs-flow:make-envelope($content, $headers, $triples, map:get($options, "dataFormat"))
  return
